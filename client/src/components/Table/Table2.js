...
import { FaWhatsapp, FaFacebook, FaGoogle, FaTag, FaTimes } from 'react-icons/fa';
...
export default function Table2({ onResults, searchval, setLeadsPdf }) {
  ...
  const updateOrientatore = async () => {
    ...
    console.log(selectedOrientatoreToChange);
        ...
        prevData.map((lead) => {
          if (lead.id === leadId) {
            const adaptedLead = {
              ...
              id: updatedLead._id,
              fequentiUni: updatedLead.frequentiUni ? updatedLead.fequentiUni : false,
  ...
  useEffect(() => {
    if (searchval == '')
      ...
      setFilteredData(
        filteredData.filter(r => r.surname.toLowerCase().includes(searchval.toLowerCase()) || r.name.toLowerCase().includes(searchval.toLowerCase()) || r.telephone.toLowerCase().includes(searchval.toLowerCase()))
      )
  }, [searchval])
  ...
  const clickEtichetta = async (scelta, data) => {
    console.log(data);
    const leadId = data.id;
    if (!data.etichette.includes(scelta)){
          try {
        const modifyLead = {
          etichetta: scelta,
          operation: 'aggiunta',
      };
      const response = await axios.put(`/lead/${userId}/update-etichette/${leadId}`, modifyLead);
      fetchLeads();
      toast.success('Il lead è stato modificato con successo.')
      console.log(response.data.message);
    } catch (error) {
  ...
  const deleteEtichetta = async (scelta, data) => {
    const leadId = data.id;
    try {
        const modifyLead = {
          etichetta: scelta,
  ...
  const clickEtichettaTable = (data) => {
  ...
  const etichettaSceltaTable = (scelta) => {
    clickEtichetta(scelta, leadSel);
    setEtichettaModify(false);
  };

  const [selectedEtichetta, setSelectedEtichetta] = useState('');

  const handleEtichettaChange = (event) => {
    const value = event.target.value;
    setSelectedEtichetta(value);
  ...
  useEffect(() => {
    if (selectedEtichetta == ''){
      ...
      const filteredData = originalData.filter((lead) => lead.etichette.includes(selectedEtichetta));
      setFilteredData(filteredData);
    }
  }, [selectedEtichetta]);

  const [nuovaEtichetta, setNuovaEtichetta] = useState('');
  const [etichetteEsi, setEtichetteEsi] = useState([]);

  const salvaEtichetta = (lead) => {
    axios
      .post('/crea-etichetta', {
        nomeEtichetta: nuovaEtichetta,
        utente: state.user._id,
        operation: 'aggiunta',
        leadId: lead.id,
      })
      .then((response) => {
        console.log('Etichetta salvata con successo:', response.data.etichetta);
        const etichettanuova = response.data.etichetta;
        //setEtichetteEsi([...etichetteEsi, etichettanuova]);
        setNuovaEtichetta("");
        fetchEtichette();
        fetchLeads();
     })
  ...
  const eliminaEtichetta = async (etichetta) => {
    axios
      .post('/crea-etichetta', {
        etichettaId: etichetta._id,
        utente: state.user._id,
        operation: 'eliminazione',
      })
      .then((response) => {
        console.log('Etichetta eliminata con successo:', response.data.etichetta);
        fetchEtichette();
     })
      .catch((error) => {
        console.error('Errore durante il salvataggio dell\'etichetta:', error);
  ...
  const fetchEtichette = async () => {
    try {
      const response = await axios.get(`/get-etichette/${state.user._id}`);
      const etichette = response.data.etichette;
      
      setEtichetteEsi(etichette);
      console.log('Etichette:', etichette);
    } catch (error) {
      console.error('Errore durante il recupero delle etichette:', error);
    }
  };
  ...
  return (
    ...
    <div className="Table-admin" style={{marginTop: '-30px'}}>
      ...
      <div className="filtralead">
        <h5 style={{ color: "gray" }}>Filtra per data o etichetta</h5>
        <div className="wrapperwrapper">
          ...
          <div className="filter-etichette">
              ...
              >
                ...
                <option value="Inviato whatsapp">Inviato Whatsapp</option>
                <option value="Da richiamare">Da richiamare</option>
                <option value="Chiude il telefono">Chiude il telefono</option>
              </select>
          </div>

          <div className="leadslinks">
            <button id="visualbtt" onClick={handletogglegrid}>
              {toggleGrid ?
                "Visualizza griglia"
                :
                "Visualizza lista"
      ...
      {calendarOpen &&
        <div className="popup-container">
          <div className="popup" id="filterbydate">
            ...
            <div style={{ cursor: "pointer", marginTop: "20px" }} onClick={() => setCalendarOpen(false)}><u>Torna indietro</u></div>
...
{orientatoriOpen &&
      ...
      {addOpen &&
      <div>
        <AddLeadPopup setAddOpen={setAddOpen} popupRef={popupRef} fetchLeads={fetchLeads} />
      </div>  
      }
      ...
      {popupModifyEsito && (
        <div className="add-lead-popup" id="popupmodifesito" ref={popupRef}>

          <div className='popup-top'>
            <h4>Modifica l'esito di {selectedLead.name}</h4>
          </div>

          <svg id="modalclosingicon" onClick={() => { setPopupModifyEsito(false); SETheaderIndex(999) }} xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 384 512"><path d="M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z" /></svg>
        ...
        <div style={{ boxShadow: "0px 0px 20px 2px #80808029", borderRadius: '20px', padding: '30px 20px', maxHeight: '58vh' }} className="table-big-container">
          <div className="table-filters">
            ...
            <div id="wrapperleadsright">
              <button onClick={handleOpenAddPopup} className="button-filter">Aggiungi Lead</button>
              <div id="leadscerca">
            ...
            <table aria-label="simple table" className="table-container">
              <thead style={{ zIndex: '5', width: '100%' }}>
                <tr>
                  ...
                  <th style={{ color: 'rgba(0, 0, 0, 0.31)', fontSize: '25px', fontWeight: "600" }}></th>
                </tr>
              </thead>
              ...
              <tbody style={{ color: "white", textAlign: 'left', padding: '0 20px' }} className="table-body-container" id="table2lista">
                {filteredData && filteredData
                  ...
                  .map((row) => (
                    <tr style={{position:'relative', padding: '0 20px'}} key={row.id}>
                      <td className="row-name">
                        {row.name + ' ' + row.surname}
                        {row.etichette && row.etichette.length > 0 ? (
                            row.etichette.map((etichetta, index) => (
                              <span
                                key={index}
                                className={`etichetta-table ${etichetta === 'Inviato whatsapp' ? 'etichetta-whats-table' : etichetta === 'Da richiamare' ? 'etichetta-rich-table' : etichetta === 'Chiude il telefono' ? 'etichetta-chiude-table' : 'etichetta-creata-visual'}`}
                              >
                                <span onClick={() => clickEtichettaTable(row)}>
                                  {etichetta}
                                </span>
                                <svg onClick={() => deleteEtichetta(etichetta, row)} xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 384 512"><path d="M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z" /></svg>
                              </span>
                            ))
                          ) : (
                            null
                          )}
                          <FaTag className='tag-etichetta-table' onClick={() => clickEtichettaTable(row)} />
                            {etichettaModify && leadSel !== null && leadSel === row ? (
                                <div className='scegli-etichetta-table'>
                                  <div>
                                    <input
                                        type="text"
                                      ...
                                      />
                                      <button
                                        onClick={() => salvaEtichetta(row)}
                                      >
                                        Salva
                                      </button>
                                  </div>
                                  <p
                                    className='etichetta-whats'
                                    onClick={() => etichettaSceltaTable('Inviato whatsapp')}
                                  >
                                    Inviato Whatsapp
                                  </p>
                                  <p
                                    className='etichetta-rich'
                                    onClick={() => etichettaSceltaTable('Da richiamare')}
                                  >
                                    Da richiamare
                                  </p>
                                  <p
                                    className='etichetta-chiude'
                                    onClick={() => etichettaSceltaTable('Chiude il telefono')}
                                  >
                                    Chiude il telefono
                                  </p>
                                  {etichetteEsi && etichetteEsi.map((etichetta, index) => (
                                    ...
                                    >
                                      <span onClick={() => etichettaSceltaTable(etichetta.nomeEtichetta)}>{etichetta.nomeEtichetta}</span>
                      ...
                      <td>
                        ...
                        >
                          ...
                          {row.status == "Venduto" ? "Venduto" : row.status}
                        </span>
                      </td>
            ...
            <div className="entries">
              {toggles.dacontattare && filteredData && filteredData.filter(x => x.status == "Da contattare").reverse().map((row, k) =>
                <LeadEntry
                  ...
                  etichette={row.etichette ? row.etichette : null}
                  modificaNelServer={clickEtichetta}
                  deleteEtichetta={deleteEtichetta}
                  etichetteEsi={etichetteEsi}
                  eliminaEtichetta={eliminaEtichetta}
                  salvaEtichetta={salvaEtichetta}
